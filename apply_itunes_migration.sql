-- Apply iTunes Matching Migration to Production Database
-- Run this in your Supabase SQL editor

-- Table to track iTunes matching attempts
CREATE TABLE IF NOT EXISTS itunes_match_attempts (
...
CREATE INDEX IF NOT EXISTS idx_itunes_match_attempts_app_id ON itunes_match_attempts(app_id);
CREATE INDEX IF NOT EXISTS idx_itunes_match_attempts_status ON itunes_match_attempts(status);
CREATE INDEX IF NOT EXISTS idx_itunes_match_attempts_created_at ON itunes_match_attempts(created_at);
...
DROP TRIGGER IF EXISTS trigger_update_itunes_match_attempts_updated_at ON itunes_match_attempts;
...
CREATE OR REPLACE VIEW itunes_matching_results AS
SELECT 
  ima.id,
  ima.app_id,
  a.name as app_name,
...
DROP POLICY IF EXISTS "Allow public read access for itunes_match_attempts" ON itunes_match_attempts;
CREATE POLICY "Allow public read access for itunes_match_attempts"
  ON itunes_match_attempts FOR SELECT
  TO anon
  USING (true);

DROP POLICY IF EXISTS "Allow public insert access for itunes_match_attempts" ON itunes_match_attempts;
CREATE POLICY "Allow public insert access for itunes_match_attempts"
  ON itunes_match_attempts FOR INSERT
  TO anon
  WITH CHECK (true);

DROP POLICY IF EXISTS "Allow public update access for itunes_match_attempts" ON itunes_match_attempts;
...
SELECT 'iTunes matching table created successfully!' as status; 